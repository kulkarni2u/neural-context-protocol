#!/usr/bin/env bash
# Claude Code SessionStart hook for the NCP plugin.
#
...
#
# Runs from the plugin's installed location (${CLAUDE_PLUGIN_ROOT}), not from
# the user's project, so it is self-contained on purpose. $CLAUDE_PROJECT_DIR
# still points at the user's project — that's where the bus's state lives.
#
...
- If ncp_lookup_memo/ncp_record_memo are available (memoization is enabled),
  ...
  with ncp_record_memo when there was no hit.

# NCP Conventions (OpenCode)

OpenCode loads `AGENTS.md` (and the files listed under `instructions` in
`opencode.json`), so this file is the "session start" for OpenCode. It makes the
NCP memory bus the agent-to-agent channel for every turn and every subagent.
...
When you dispatch any subagent (a sub-task, `ncp handoff`, or another agent),

...
def test_needle_recall_reports_any_deficit_honestly(tmp_path: Path) -> None:
    artifact = needle_recall(
    ...
    ncp = float(final["ncp"])
    sliding = float(final["sliding_window"])
    assert ncp >= sliding or artifact["summary"]["reported_deficit"] is True
